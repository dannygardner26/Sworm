...
export class ProviderRegistry {
  ...
  private registerDefaultFactories(): void {
    ...
    this.factories.set('azure-openai', async (name, config, model) => {
      ...
      return new OpenAIProvider(
        name,
        config.apiKey!,
        model ?? config.deployment ?? config.defaultModel ?? 'gpt-4o',
        config.baseUrl,
      );
    });
    ...
    this.factories.set('bedrock', async (name, config, model) => {
      ...
      return new BedrockProvider(
        ...
        model ?? config.defaultModel ?? 'anthropic.claude-sonnet-4-6-20250514-v1:0',
      );
    });
    this.factories.set('ollama', async (name, config, model) => {
      ...
      return new OllamaProvider(
        name,
        model ?? config.defaultModel ?? 'llama3.1',
  ...
  async get(providerName: string, modelOverride?: string): Promise<LLMProvider> {
    ...
    if (!providerConfig)
      throw new Error(`Unknown provider: ${providerName}. Check ~/.sworm/providers.yaml`);
